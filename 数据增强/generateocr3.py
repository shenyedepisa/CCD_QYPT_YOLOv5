...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dst', type=str, default='./data-non/images_gc3', help='destination path')
    ...
    for fileName in fileList:
        ...
        fnt1 = ImageFont.truetype(r'./fonts/chinese.simfang.ttf',50)
        fnt2 = ImageFont.truetype(r'./fonts/chinese.simhei.ttf',60)
        fnt3 = ImageFont.truetype(r'./fonts/chinese.stfangso.ttf',70)
        fnt4 = ImageFont.truetype(r'./fonts/stkaiti.ttf',80)
        fnt5 = ImageFont.truetype(r'./fonts/chinese.stsong.ttf',90)
        fnt6 = ImageFont.truetype(r'./fonts/chinese.stxihei.ttf',50)
        fnt7 = ImageFont.truetype(r'./fonts/chinese.stxinwei.ttf',60)
        fnt8 = ImageFont.truetype(r'./fonts/chinese.stzhongs.ttf',70)
        fnt9 = ImageFont.truetype(r'./fonts/chinese.stsong.ttf',80)
        fnt10 = ImageFont.truetype(r'./fonts/simyou.ttf',90)

        draw1.text((0.1*im.size[0], 0.05*im.size[1]), u'密', fill='red', font=fnt1)
        draw2.text((0.2*im.size[0], 0.05*im.size[1]), u'密', fill='black', font=fnt2)
        draw3.text((0.3*im.size[0], 0.05*im.size[1]), u'密', fill='blue', font=fnt3)
        draw4.text((0.4*im.size[0], 0.05*im.size[1]), u'密', fill='red', font=fnt4)
        draw5.text((0.5*im.size[0], 0.05*im.size[1]), u'密', fill='black', font=fnt5)
        draw6.text((0.05*im.size[0], 0.2*im.size[1]), u'密', fill='red', font=fnt6)
        draw7.text((0.05*im.size[0], 0.3*im.size[1]), u'密', fill='black', font=fnt7)
        draw8.text((0.05*im.size[0], 0.4*im.size[1]), u'密', fill='purple', font=fnt8)
        draw9.text((0.05*im.size[0], 0.5*im.size[1]), u'密', fill='red', font=fnt9)
        draw10.text((0.05*im.size[0], 0.6*im.size[1]), u'密', fill='black', font=fnt10)
        ...
        label_path = (args.src).replace('images','labels_gc3')
        label = img_name.replace('images','labels_gc3')
        ...
        centerx1 = (0.1*im.size[0]+40/2)/im.size[0]
        centerx2 = (0.2*im.size[0]+40/2)/im.size[0]
        centerx3 = (0.3*im.size[0]+70/2)/im.size[0]
        centerx4 = (0.4*im.size[0]+80/2)/im.size[0]
        centerx5 = (0.5*im.size[0]+90/2)/im.size[0]
        centerx6 = (0.05*im.size[0]+50/2)/im.size[0]
        centerx7 = (0.05*im.size[0]+40/2)/im.size[0]
        centerx8 = (0.05*im.size[0]+70/2)/im.size[0]
        centerx9 = (0.05*im.size[0]+80/2)/im.size[0]
        centerx10 = (0.05*im.size[0]+40/2)/im.size[0]

        centery1 = (0.05*im.size[1]+40/2)/im.size[1]
        centery2 = (0.05*im.size[1]+40/2)/im.size[1]
        centery3 = (0.05*im.size[1]+90/2)/im.size[1]
        centery4 = (0.05*im.size[1]+100/2)/im.size[1]
        centery5 = (0.05*im.size[1]+115/2)/im.size[1]
        centery6 = (0.2*im.size[1]+75/2)/im.size[1]
        centery7 = (0.3*im.size[1]+40/2)/im.size[1]
        centery8 = (0.4*im.size[1]+90/2)/im.size[1]
        centery9 = (0.5*im.size[1]+100/2)/im.size[1]
        centery10 = (0.6*im.size[1]+40/2)/im.size[1]
        ...
        height0 = 40/im.size[1]

        file1.write('0 ' + str(centerx1) + ' ' + str(centery1) + ' ' + str(width0) + ' ' + str(height0) + '\n')
        file2.write('0 ' + str(centerx2) + ' ' + str(centery2) + ' ' + str(width0) + ' ' + str(height0) + '\n')
        ...
        file7.write('0 ' + str(centerx7) + ' ' + str(centery7) + ' ' + str(width0) + ' ' + str(height0) + '\n')
        ...
        file10.write('0 ' + str(centerx10) + ' ' + str(centery10) + ' ' + str(width0) + ' ' + str(height0) + '\n')
